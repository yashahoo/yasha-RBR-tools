...
fname='WA_latest_BOM_obs_WEBLINKS.csv'
...
keep infile startline endline si recent_dir old_dir D recent site_num Name
...
try
atime=archive(si).mtime_UTC;
if isempty(atime)
    disp(['atime is empty for site: ' num2str(si)])
end
ltime=atime(end);
...
if dtgap>0.51
    disp(['Data gap of ' num2str(dtgap) ' hrs'])    
end
 ...
 f = fieldnames(recent);
 for i = 1:length(f)
    combined(si).(f{i}) = [archive(si).(f{i});recent(si).(f{i})(rti)];
 end

 catch
    disp(['NO NEW DATA AVAILABLE FOR SITE: '  num2str(si)   ])
    f = fieldnames(recent);
    for i = 1:length(f)
    combined(si).(f{i}) = archive(si).(f{i});
    end
end
...
setpref('Internet','SMTP_Password','Tombies9254');
...
% then use this
sendmail('yashahoo@gmail.com','Error downloading BOM files',['Error downloading Bom data on ' datestr(now) ' ' err])
...
copyfile('BOM_obs_archived.mat', '/home/yasha/public_html/')
copyfile('SA_BOM_obs_archived.mat', '/home/yasha/public_html/')
disp('FINISHED COPYING to /home/yasha/public_html/')
