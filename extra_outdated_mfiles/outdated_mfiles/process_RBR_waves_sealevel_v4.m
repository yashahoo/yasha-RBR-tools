...
% location='OldDunsborough'; rawfile='/Volumes/Margs_Clone/RBRpressure_sensors/20220818_Busselton_RBR_ABB_DUNS_EB_StuBarr/DUNS_206857_20220818_1312.rsk'
...
% location='BusseltonJetty341'; rawfile='/Volumes/Margs_Clone/RBRpressure_sensors/Busselton_Jetty_202208/208787_20220911_1406.rsk' % end of jetty
...
% figure; 
% plot(ts,p,'r')
%% interactively select start and finish times if desired...
...
%%
% It makes a big difference in waters >  5m (Cockburn Sound and Busselton Jetty).
% The problem is that ideally it is an iterative process:
% (1) Calculate wavenumber (need period and mean depth) and then correct the depth record
% Or calculate the wave height and then correct for the pressure response

% Did work – not we do have a another issue – if we have the sensor close to the surface it is not an issue but when it is more than about 2m deep have to correct for the attenuation of the pressure signal – should do for all anyway.
%  
% Background: https://oceanlyz.readthedocs.io/en/stable/5_Apply_Pressure_Response_Factor.html
%  
% sigma=2*pi/wavestata(8,i);
% h=mean(data1(spoint:epoint,1));
% k= wavenumber(sigma,h);
% pr=1/cosh(k*h);
% wavestata(29,i)=pr;
%  
% we have to multiply the wave height by pr
...
% k=a1./h;
%   
%% test new wave program
% 
% addpath(genpath('/Users/00068592/Documents/MATLAB/m-files/oceanlyz_2_0'))
% 
