...
% 
% ** this  version is adaptd to work on PC and uses '\'  instead of '/' for
% file paths
% 
% last modified
% 20180529
...
% set the path to the scripts here relevant to your computer
addpath(genpath('yasha_RBR_tools')); % [edit]  this should contain all the functions you need
addpath(genpath('yasha_RBR_tools/rbr-rsktools')); % use this old version to ensure it reads temperature!

%%  ------------------------settings-------------------------------------- %

% set complete path to raw data file .rsk
rawfile='/Users/Yasha/Documents/RESEARCH/DATA/MEASURED/RBRpressure_sensors/SN124029_Gracetown_offshore20180519_1928/SN124029_Gracetown_offshore20180519_1928.rsk' % [edit] complete path to raw .rsk file
location='Gracetown_Offshore'; % descriptive name used for filenaming % [edit] 

% set where to save figures and data
whereput=[rawfile(1:end-4) '_processed'];  %[edit]

% deployment info [ default]
sample_rate_hz=2; % sampling in hz (normally 2)
max_period=25; % seconds for cutoff in calculating waves

% define start and end times of good data  or leave as NaN to interactively select [edit]
% begin=datenum(2018,1,14,9,30,0); % start of good data (can get from running initial reading of data in next section)
% finish=datenum(2018,2,27,12,10,0); % end of good data (can get from running initial reading of data in next section)
begin=NaN; % leave as nan if want to interactively select
finish=NaN;
...
plot_highpass_weeks='N'  % 'Y' if you want to plot weekly [edit]

% save the processed data to matfile [edit]
save_matfile='Y'

% set font size for figs
fs=12; 
