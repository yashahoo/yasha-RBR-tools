...
start_WL_datum_ht=1.54-0.95+0.39 ; % [ht of deck - rbr to deck + rbr to water surface] %round_start_finish must  be = 'N' !!!;  this to be added to the MSL zero height for first 30*sample_rate_hz observations. Make Nan if unknown or don't want to be chart datum
...
if ~isnan(start_WL_datum_ht)
    ...
    p=p_CD;
    datum='CD';
    datum_correction=-dp+start_WL_datum_ht
    disp('water level /pressure adjusted to chart datum')
    trends_removed=Tr.OutputSlope.*p+Tr.OutputOffset;
...
%         ncwrite(ncref,'trimmed_time',trimmed_time-datenum('2000-01-01'));
    ...
    case 'Y'
        ...
        ncwriteatt(ncref,'corrected_press','description','pressure corrected for depth https://oceanlyz.readthedocs.io/en/latest/');
