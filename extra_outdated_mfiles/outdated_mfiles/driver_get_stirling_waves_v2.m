...
% requirements: import_pressure_logfiles.m, updated autospec.m 
...
years2do = [2017] % can be vector of years (for loop)
months2do = [7]   % can be vector of months (e.g. [1:12])

input_directory='/Users/00068592/Documents/RESEARCH/OTHER_PEOPLE/CHARI/Cockburn_pressure_sensor/'
output_directory=[input_directory filesep 'processed_Stirling_waves']
...
use_corrected_sealevel='N' % 'Y' if you want to use data corrected for pressure response in ocn_toolbox and chari_method, 'N' otherwise. Must check detailed ocn settings below to make sure water depth and sensor ht are correct.

% [requires both if calculate_waves_both_methods = 'N' above]
calculate_waves='chari_method'
...
%         fdir='/Users/00068592/Documents/RESEARCH/OTHER_PEOPLE/CHARI/Cockburn_pressure_sensor/'
        ...
        d=dir([fdir num2str(year) '-' sprintf('%02d',month) '*.log'])
        
        dat=[];ts=[];p=[];co=[];

        
% make data matrices to fill
logfilename= cell(length(d),1)
...
num_infiles=length(d);
        ...
        for fi=1:length(d);
            ...
            %%-------------------------------------------------------------------------%
            % calculate_waves='ocn_toolbox'
            InputType='waterlevel' %'pressure' %'waterlevel' %'pressure'              % Data input ['pressure'] or  'waterlevel'. If pressure, the signal attenuation with depth will be applied [recommended].
            OutputType='wave' %'wave+waterlevel' %'wave' %'wave+waterlevel' %'wave+waterlevel'      % ['wave'], ['wave+waterlevel'];
...
ocn_colnames={'logfilename','ocn_number_samples','ocn_Time','ocn_Hs','ocn_Hm0','ocn_Tp','ocn_Hm0swell','ocn_Tpswell','ocn_Hm0sea','ocn_Tpsea','ocn_fp','ocn_fseparation'};
...
%                     if possible use corrected pressure record
                switch use_corrected_sealevel
                    ...
                    ERRMSG=['Error using corrected pressure!! ocn InputType = sealevel, change to pressure and re-run'];
                    error(ERRMSG)
                    end
                end
                ...
                case 'chari_method'
                    ...
                    switch burst_duration
                        case 'All'
                            ...
                            burst_duration= length(ts)/srate%3599 %3600 15*60 ;       % Interval of time window (in seconds) over which to calculate waves (must be <= number of samples in file)
                            disp(['Using complete data range for burst_duration = ' num2str(burst_duration)])
                    end
...
%     Hs 0.14374
 ...
 switch calculate_waves_both_methods
     case 'Y'
     combined_colnames={'logfilename','number_samples','Time','Hs','Hrms','Tpeak','Inf_ht','Swell_ht','Wind_ht','Inf_Per','Swell_Per','Wind_Per','ocn_Hs','ocn_Hm0','ocn_Tp','ocn_Hm0swell','ocn_Tpswell','ocn_Hm0sea','ocn_Tpsea','ocn_fp','ocn_fseparation'};   
         T=table(logfilename,number_samples, {datestr(Htime,'yyyy-mm-dd HH:MM:ss')},Hs,Hdat(1),Hdat(2),Hdat(3),Hdat(4),Hdat(5),Hdat(6),Hdat(7),Hdat(8),ocn_Hs,ocn_Hdat(1),ocn_Hdat(2),ocn_Hdat(3),ocn_Hdat(4),ocn_Hdat(5),ocn_Hdat(6),ocn_Hdat(7),ocn_Hdat(8),'VariableNames',combined_colnames)

          outfname=[output_directory filesep num2str(year) sprintf('%02d',month) '_wave_data-ocn_toolbox-and-chari_method_SL_corr_' use_corrected_sealevel];
          save(outfname,'T','num_infiles');
          % write them  with less decimals in clunky way
          Tstr=table(logfilename,number_samples, {datestr(Htime,'yyyy-mm-dd HH:MM:ss')},{sprintf('%0.3f',Hs)},{sprintf('%0.3f',Hdat(1))},{sprintf('%0.3f',Hdat(2))},{sprintf('%0.3f',Hdat(3))},{sprintf('%0.3f',Hdat(4))},{sprintf('%0.3f',Hdat(5))},{sprintf('%0.3f',Hdat(6))},{sprintf('%0.3f',Hdat(7))},{sprintf('%0.3f',Hdat(8))},{sprintf('%0.3f',ocn_Hs)},{sprintf('%0.3f',ocn_Hdat(1))},{sprintf('%0.3f',ocn_Hdat(2))},{sprintf('%0.3f',ocn_Hdat(3))},{sprintf('%0.3f',ocn_Hdat(4))},{sprintf('%0.3f',ocn_Hdat(5))},{sprintf('%0.3f',ocn_Hdat(6))},{sprintf('%0.3f',ocn_Hdat(7))},{sprintf('%0.3f',ocn_Hdat(8))},'VariableNames',combined_colnames);
     ...
     case 'N'
         switch calculate_waves
             case 'ocn_toolbox'
                 % ocn_colnames={'logfilename','ocn_number_samples','ocn_Time','ocn_Hs','ocn_Hm0','ocn_Tp','ocn_Hm0swell','ocn_Tpswell','ocn_Hm0sea','ocn_Tpsea','ocn_fp','ocn_fseparation'};
                 T=table(logfilename,ocn_number_samples, {datestr(ocn_Htime,'yyyy-mm-dd HH:MM:ss')},ocn_Hs,ocn_Hdat(1),ocn_Hdat(2),ocn_Hdat(3),ocn_Hdat(4),ocn_Hdat(5),ocn_Hdat(6),ocn_Hdat(7),ocn_Hdat(8),'VariableNames',ocn_colnames )
                 outfname=[output_directory filesep num2str(year) sprintf('%02d',month) '_wave_data_' calculate_waves '_SL_corr_' use_corrected_sealevel];
                 save(outfname,'T','num_infiles');
% write them  with less decimals in clunky way 
                 Tstr=table(logfilename,ocn_number_samples, {datestr(ocn_Htime,'yyyy-mm-dd HH:MM:ss')},{sprintf('%0.3f',ocn_Hs)},{sprintf('%0.3f',ocn_Hdat(1))},{sprintf('%0.3f',ocn_Hdat(2))},{sprintf('%0.3f',ocn_Hdat(3))},{sprintf('%0.3f',ocn_Hdat(4))},{sprintf('%0.3f',ocn_Hdat(5))},{sprintf('%0.3f',ocn_Hdat(6))},{sprintf('%0.3f',ocn_Hdat(7))},{sprintf('%0.3f',ocn_Hdat(8))},'VariableNames',ocn_colnames);
                 writetable(Tstr,outfname,'Delimiter',',','QuoteStrings',true)
             ...
             case 'chari_method'
                 ...
                 save(outfname,'T','num_infiles');
% write them  with less decimals in clunky way
                 Tstr=table(logfilename,ocn_number_samples, {datestr(ocn_Htime,'yyyy-mm-dd HH:MM:ss')},{sprintf('%0.3f',Hs)},{sprintf('%0.3f',Hdat(1))},{sprintf('%0.3f',Hdat(2))},{sprintf('%0.3f',Hdat(3))},{sprintf('%0.3f',Hdat(4))},{sprintf('%0.3f',Hdat(5))},{sprintf('%0.3f',Hdat(6))},{sprintf('%0.3f',Hdat(7))},{sprintf('%0.3f',Hdat(8))},'VariableNames',colnames);
                 writetable(Tstr,outfname,'Delimiter',',','QuoteStrings',true);
         end
 end
