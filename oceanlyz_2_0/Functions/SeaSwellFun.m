...
% yh  - when f is unrealistic compared to fseparation
separate_failed='N'; %yh
if isempty(loc2) | loc2==0
    loc2=length(find(f<=1/8));
    disp('SEPARATION FAILED....CUTTING SEA / SWELL AT 8 SECS')
    separate_failed='N'; % if you make this 'N' it will keep the value calculated with 8 sec cutoff (maybe junk) if you make = 'Y'; it will fill with nans.
% keyboard
pause(2)
end

% separate_failed='N'; %yh
% try %yh this catch is for when waves are very tiny and it fails to find separation point
...
switch separate_failed % mask with NaN where I have falsely separated the sea/swell at 8 secs
    case 'N'
        %calculating swell and sea
        
        % loc2=length(find(f<=fseparation)); %location of fseperation % old one yh
    ...
    case 'Y'
        m0swell=NaN;
        m0sea=NaN; %Zero-Moment wave height
        Hm0sea=NaN; %sea Zero-Moment wave height
        Hm0swell=NaN; %swell Zero-Moment wave height
        Tpsea=NaN; %sea peak period, loc4+loc5-1 is the location of sea peak 
        Tpswell=NaN; %swell peak period, loc4+loc5-1 is the location of swell peak
        fpsea=NaN; %sea peak frequency based on fseparation from previous step
        fpswell=NaN;
end
%--------------------------------------------------------------------------
%Displaying results
