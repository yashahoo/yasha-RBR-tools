...
months2do = [2] %[7 8]   % can be vector of months (e.g. [1:12] for loop)
...
min_threshold_good=0.99 % keep file if >99% good data

remove_outliers='Y'     % replace outliers with nans? This uses moving window to identify outliers.... makes it much slower.
outlier_stds=4          % use this number of standard deviations from mean to identify bad data [4 is good]
...
maxgap_secs=3600        % maximum gap in seconds to interpolate across
...
burst_duration=3500 %'All';  % Interval of time window (in seconds) over which to calculate waves (must be <= number of samples in file) don't make exactly one hour as final point often missing and then misses last hour
...
%         logfilename= cell(length(d),1); % this doesnt work!!
        logfilename= repmat({''},num_hrs,1);
        number_samples= NaN(num_hrs,1);
        Htime= NaN(num_hrs,1);
        Hdat=  NaN(num_hrs,8);
        Hs=  NaN(num_hrs,1);
        % for ocn
        ocn_number_samples= NaN(num_hrs,1);
        ocn_Htime= NaN(num_hrs,1);
        ocn_Hs=NaN(num_hrs,1);
        ocn_Hs_uncorrected=NaN(num_hrs,1);
        ocn_Hdat=NaN(num_hrs,8);
        
        num_infiles=length(d);
        
        lasthco=0;
        for fi=1:length(d); % loop through the files
            
            co=co+1; % counter for files
...
%                     try to get outliers when instrument changed over
            ...
            srate=sRateHz; % sample rate in hertz  [for chari method]
            
%             burst_duration= floor(length(ts)/srate); %2048 %3598 % 3600 %length(ts)/srate ( this was for hourly data)
...
%             calculate_waves='ocn_toolbox' % [comment out if using chari method]
            ...
            switch calculate_waves
                case 'ocn_toolbox'
                    ...
                    %  save the data for each day [xxxthe monthxxx]
                    %                     logfilename{co} = d(fi).name ;
                    %  counters for hour indices
                    hco=[lasthco+1:lasthco+size(ocn.wave.Burst_Data,1)];
                    lasthco=lasthco+size(ocn.wave.Burst_Data,1);
                    
                    ocn_number_samples(hco)=repmat(size(ocn.wave.Burst_Data,2),length(hco),1);
                    ocn_Htime(hco) = burst_times ; %mean(ts);   %'yyyy-mm-dd HH:MM:ss.fff'
                    ocn_Hs_uncorrected(hco)=4*std(ocn.wave.Burst_Data,0,2,'omitnan'); % using uncorrected data
                    
                    if strcmpi(InputType, 'pressure');
%                         ocn_Hs(hco)=4*std(ocn.wave.Eta); % using corrected data
                        ocn_Hs(hco)=4*std(ocn.wave.Eta,0,2,'omitnan'); % using corrected data
                    elseif strcmpi(InputType, 'waterlevel');
                        ocn_Hs(hco)=4*std(ocn.wave.Burst_Data,0,2,'omitnan');
                    end
                    
                    ocn_Hdat(hco,:)= [ocn.wave.Hm0  ocn.wave.Tp ocn.wave.Hm0swell ocn.wave.Tpswell ocn.wave.Hm0sea ocn.wave.Tpsea  ocn.wave.fp  ocn.wave.fseparation];
...
figure; 
plot(T.ocn_Time,T.ocn_Hm0swell);ylim([0 .6]);datetick;set(gca,'fontsize',14)
hold on; plot(T.ocn_Time,T.ocn_Hm0sea)
% plot(T.ocn_Time,T.ocn_Inf_ht)
% plot(T.ocn_Time,T.ocn_Hs)
legend('Swell_ht','Wind_ht','Inf_ht','Hs','interpreter','none')
ylabel('Height (m)')
datetick
