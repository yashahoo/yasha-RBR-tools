...
% added verbosity
% 20230412 yh
% - added check to correctly read which channel  temperature and pressure
%   are stored in to work with new sensors
%%
% % clear all
% % close all
% % clc;
...
% find which channel is temperature and find which is pressure (rsktools-3.5.3)
    Pidx = find(strcmpi({data1.channels.longName},'Pressure'))
    Tidx = find(strcmpi({data1.channels.longName},'Temperature'))
% data1.channels.longName
...
%     
%     data1.data.values(:,Pidx)=data1.data.values(:,Pidx)-10.1325;
%     
% else
%     corrected_with_mslp_obs='N';
% data1.data.values(:,Pidx)=data1.data.values(:,Pidx)-10.1325;
data1.data.values(:,Pidx)=data1.data.values(:,Pidx);
% end
...
figure;
hold on
plot(data1.data.tstamp,data1.data.values(:,Pidx),'r') %if using new  rsktools-3.5.3
ylabel([data1.channels(Pidx).longName ' (' data1.channels(Pidx).units ')'])
% plot(data1.data.tstamp,data1.data.values(:,Pidx)-mean(data1.data.values(:,Pidx)),'r') %if using new  rsktools-3.5.3
...
if ~isempty(Pidx)
try
p1=data1.data.values(:,Pidx);
disp(['Pressure data exists in channel ' num2str(Pidx)])
catch
    disp('No Pressure channel detected in data file...')
    return
end
end

if ~isempty(Tidx)
temp1=data1.data.values(:,Tidx);
disp(['Temperature data exists in channel ' num2str(Tidx)])

else
    disp('No temperature data exist!!!')
end
